...
The model kernels are the 207 generated compute shaders under
`generated-models/hlsl`. They preserve the official model weights and pass
math from the pinned Anime4K sources under `third_party`.
`generated-models/manifest.json` defines all resource bindings, dimension and
WHEN expressions, 13 effects, and all 18 native presets.
...
All 207 kernels build with FXC `/Ges /WX /O3`; a D3D11 WARP test executes every
one of the 18 preset graphs and checks its output. Exact floating-point pixels
