...
describe('fullscreen shadow style restoration', () => {
  it('restores only properties that still contain the extension value', () => {
    const manager = Object.create(FullscreenLayoutManager.prototype) as any;
    ...
    const snapshots = manager.applyTemporaryStyles(element, {
      position: 'fixed', width: '100vw', height: '100vh',
    });
    style.setProperty('height', '80vh');
    manager.restoreTemporaryStyles(element, snapshots);
